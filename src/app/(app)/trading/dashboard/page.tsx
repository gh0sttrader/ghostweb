...
const PositionRow = ({ position, stock, onSelect, isSelected }: { position: any, stock: Stock | undefined, onSelect: (symbol: string) => void, isSelected: boolean }) => {
    if (!stock) {
        return (
            <div className="flex justify-between items-center text-sm border-b border-white/10 last:border-b-0 py-2">
                ...
                <div className="text-right text-muted-foreground">Loading...</div>
            </div>
        );
    }
    ...
    return (
        <div 
            className={cn(
                "flex justify-between items-center text-sm border-b border-white/10 last:border-b-0 py-2 px-2 rounded-md cursor-pointer hover:bg-white/5",
                                      ...
                                      <div className="space-y-2">
                                          {accountPositions.map(pos => (
                                              <PositionRow
                                                  ...
                                                  isSelected={syncedTickerSymbol === pos.symbol}
                                              />
                                          ))}
                                      </div>
                                  ) : (
